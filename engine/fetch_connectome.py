...
neurotransmitter type), classification.csv.gz (neuron ID -> FlyWire's own
super_class/class annotation), and neurons.csv.gz (neuron ID -> FlyWire's
finer-grained `group` neuropil label, plus its own per-neuron dominant
neurotransmitter classification `nt_type` and per-NT confidence averages).
See flybreak/README.md, "The real connectome" for what network.py does
with them and why.
...
_FILES = ["connections.csv.gz", "classification.csv.gz", "neurons.csv.gz"]
